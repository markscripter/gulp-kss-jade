...
module.exports = function(opt) {
  ...
  templatePath = path.resolve(process.cwd() + '/' + opt.template);
  ...
  function processKss() {
    ...
    kss.parse(buffer, opt, function(err, guide) {
      ...
      sectionCount === 0 ?
        this.emit('error', new PluginError(PLUGIN_NAME, 'No KSS documentation discovered in source files.')) :
        0;
      ...
      for (i = 0; i < rootCount; i += 1) {
        ...
        _this.emit('data', generatePage(styleguide, childSections, sectionRoots[i], sectionRoots, template));
      }

      // Generate the homepage.
      childSections = [];
      _this.emit('data', generatePage(styleguide, childSections, 'styleguide.homepage', sectionRoots, template));
  ...
  function generatePage(styleguide, sections, root, sectionRoots, template) {
    ...
    var filename = path.join(firstFile.base, filename);
    var file = new File({
      cwd: firstFile.cwd,
      base: firstFile.base,
      path: filename,
  ...
  var emitEnd = underscoreAfter(2, function emitEnd(_this) {
    _this.emit('end');
  });
